...
# Create the latent dirichlet allocation transformation 
lda = LdaModel(corpus_tfidf, num_topics=100)
...
# Create and train the same classifier as SGD.py
clf = OneVsRestClassifier(SGDClassifier(random_state=SEED), n_jobs=-1)
clf.fit(lsi2scipy(corpus_lda), labels)
       
labels = mlb.transform(test_labels)

# Do the same transformations for the test set
tokens = vect.transform(test['Body'])
corpus = Sparse2Corpus(tokens.T)
corpus_tfidf = tfidf[corpus]
corpus_lda = lda[corpus_tfidf]

# Predict the classes of the tokens
predicted = clf.predict(lsi2scipy(corpus_lda))
